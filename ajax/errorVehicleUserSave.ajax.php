...
echo "deliveryDate[".$deliveryDate;
echo ",";
echo "meridiemType[".$meridiemType;
echo ",";
echo "meridiemFlag[".$meridiemFlag;
echo ",";
echo "locationId[".$locationId;
echo ",";
echo "ci_guestId[".$ci_guestId;
...
// 오류건은 고객정보 오류인 상태로 유지합니다.
// 오류건엥 대해서 경로를 새로 잡을려고했는데, 중복배송건이 있을경우 오류건에 대한 추가 경로 배정시 문제가 되기때문에,
// 만약 이 오류건에 대해서 ERP측으로부터 정상적으로 수정되어 반영된다면, 오류건 리스트에는 더 이상 떠지 않습니다.
...
for($i=0;$i<count($vehicleErrorUserList);$i++){
	...
	if($vehicleErrorUserList[$i]['guestLat'] > 0 &&$vehicleErrorUserList[$i]['guestLon'] > 0){
...
for($i=0;$i<count($vehicleErrorUserList);$i++){
	...
	if($vehicleErrorUserList[$i]['guestLat'] > 0 && $vehicleErrorUserList[$i]['guestLon'] > 0){
		unset($DATA);
		$WHERE = " WHERE 1=1
						AND ve_accno			= '".$vehicleErrorUserList[$i]['accno']."'
		...
		$DATA["ve_isJuso"]			= "N";											// 주소 체크
		$DATA["ve_errorJusoFlag"]	= "Y";											// 에러주소 플래그
